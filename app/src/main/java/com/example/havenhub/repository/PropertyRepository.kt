...
class PropertyRepository @Inject constructor(
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Read — Saari properties (no filter)
    // ─────────────────────────────────────────────────────────────────────────

    // ✅ FIX: Koi filter nahi — saari properties return karo
    ...
    suspend fun getFeaturedProperties(): Resource<List<Property>> {
        ...
        val all = (result as Resource.Success).data ?: emptyList()
        val featured = all.filter { it.isFeatured }
        return Resource.Success(if (featured.isEmpty()) all.take(5) else featured)
    }
